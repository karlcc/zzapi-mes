...
import { ZzapiMesClient, ZzapiMesHttpError, ensureProtocol } from "../index.js";
...
describe("ZzapiMesClient", () => {
  ...
  it("getPo builds correct URL with ebeln and sap-client", async () => {
    ...
    await new ZzapiMesClient(CFG).getPo("3010000608");
  ...
  it("throws ZzapiMesHttpError on error response", async () => {
    ...
    const c = new ZzapiMesClient(CFG);
  ...
  it("strips trailing slash from host", async () => {
    globalThis.fetch = mockFetch(200, '{"ok":true,"sap_time":"20260422163000"}');
    await new ZzapiMesClient({ ...CFG, host: "http://sapdev.test:8000/" }).ping();
